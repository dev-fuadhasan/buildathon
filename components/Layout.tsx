...
export default function Layout({ children }: Props) {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${pathname === "/chat" ? "h-dvh overflow-hidden bg-white" : "bg-white"}`}>
      {/* Header - Enhanced Navigation */}
      <header className={`${pathname === "/chat" ? "hidden" : "bg-white/80 backdrop-blur-md border-b border-neutral-200 sticky top-0 z-50 transition-all duration-300"}`}>
      ...
      {/* Main Content */}
      <main className={`${pathname === "/chat" ? "flex-1 min-h-0 overflow-hidden" : "flex-1 flex flex-col"}`}>
        {isHome || pathname === "/chat" || isAuthPage ? (
          children
        ) : (
